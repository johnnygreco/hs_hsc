; hs_radec2ds9 is a Modified version of ds9regions
;+
; NAME:
;   ds9regions
;
...
PRO hs_radec2ds9, filename, ra, dec, shape=shape, boxWidth=boxWidth, $
    boxHeight=boxHeight, boxAngle=boxAngle, circleRadius=circleRadius, $
    color=color, text=text, width=width, B1950=b1950, APPEND=append, $
    SEXIGESIMAL=sexigesimal, RESIZEABLE=resizeable, MOVEABLE=moveable
  ...
  for i=0, n_elements(ra)-1 do begin
     ...
     if keyword_set(WIDTH) then begin
        width1 = (n_elements(WIDTH) lt n_elements(ra)) ? width[0] : width[i]
     endif
     ...
     if keyword_set(WIDTH) then command=command+'width='+width1+' '
